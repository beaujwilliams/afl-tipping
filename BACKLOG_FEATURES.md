...
| BL-018 | P1 | Idea | Server-render leaderboard first, then lazy-load trends and private groups | Biggest real load-time improvement on the slowest high-traffic page | L | Keep ranking logic identical; move only initial read path to server and hydrate secondary views after paint |
| BL-019 | P1 | Idea | Server-render results pages instead of loading after mount | Removes blank/loading phase on results index and round detail pages | L | No scoring changes; server-load the same data and keep recap/admin actions client-side |
