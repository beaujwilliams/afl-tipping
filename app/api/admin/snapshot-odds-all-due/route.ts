...
export async function GET(req: Request) {
  try {
    ...
    // ✅ IMPORTANT: your schema uses round_number + lock_time_utc
    let q = supabase
      .from("rounds")
    ...
    if (error) return NextResponse.json({ error: "Failed to read rounds", details: error.message }, { status: 500 });
    if (!rounds?.length) return NextResponse.json({ ok: true, season, processedDueRounds: 0, capturedRounds: 0, results: [] });
    ...
    for (const r of rounds) {
      ...
      if (!due) {
        results.push({ round: r.round_number, due: false, snapshotForTimeUtc: r.lock_time_utc, note: "Not due yet" });
        continue;
      }
