...
function toOddsApiUtc(isoWithMs: string) {
  // "2026-03-12T02:30:00.000Z" -> "2026-03-12T02:30:00Z"
  return isoWithMs.replace(".000Z", "Z").replace(/\.\d{3}Z$/, "Z");
}
...
export async function GET(req: Request) {
  ...
  if (!season || Number.isNaN(season) || round === undefined || Number.isNaN(round)) {
  ...
  // Build date window (UTC) for this round
  const times = matches
  ...
  for (const match of matches) {
    ...
    const best = candidates
      .map((e) => ({ e, diff: Math.abs(new Date(e.commence_time).getTime() - targetTime) }))
    ...
    const homeOutcome = outcomes.find((o) => normTeam(o.name) === normTeam(match.home_team));
    const awayOutcome = outcomes.find((o) => normTeam(o.name) === normTeam(match.away_team));
