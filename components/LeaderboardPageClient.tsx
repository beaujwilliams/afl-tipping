...
function normalizeLeaderboardState(json: LeaderboardResponse | null | undefined) {
  ...
  if (
    ...
    !championHighlightUserIds.includes(reigningChampionUserId)
  ) {
    championHighlightUserIds.unshift(reigningChampionUserId);
  }

  const trendRounds = Array.isArray(json?.scored_rounds)
    ? json.scored_rounds
  ...
  const trendSeries = Array.isArray(json?.rank_trends)
...
function LeaderboardTrendLoadingSkeleton({ isMobile }: { isMobile: boolean }) {
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      >
        <UiSkeleton width="100%" height="100%" radius={10} />
      </div>

      <div style={{ display: "grid", gap: 10 }}>
        <UiSkeleton width="100%" height={40} radius={10} />
        <div
          style={{
            border: "1px solid var(--border)",
            borderRadius: 12,
        ...
        >
          {Array.from({ length: 6 }).map((_, index) => (
            <div
              key={`trend-option-skeleton-${index}`}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 8,
            ...
            >
              <UiSkeleton width={14} height={14} radius={4} />
              <UiSkeleton width={9} height={9} radius={999} />
              <UiSkeleton width={index % 2 === 0 ? "38%" : "52%"} height={16} />
              <UiSkeleton width={36} height={12} style={{ marginLeft: "auto" }} />
            </div>
          ))}
        </div>
      </div>
...
export default function LeaderboardPageClient({
  ...
  const [selectedTrendUserIds, setSelectedTrendUserIds] = useState<string[]>(
    initialLeaderboardState.selectedTrendUserIds
  );
  ...
  const [loadingGroups, setLoadingGroups] = useState(initialViewMode === "groups");
  ...
  function applyLeaderboardData(json: LeaderboardResponse) {
    ...
    setSelectedTrendUserIds((prev) => {
      const validIds = new Set(normalized.trendSeries.map((series) => series.user_id));
      const kept = prev.filter((userId) => validIds.has(userId));
      if (kept.length > 0) return kept;
      return normalized.selectedTrendUserIds;
    });
  }
  ...
  useEffect(() => {
    ...
    if (viewMode !== "groups" && !currentGroupQuery) return;
    void loadGroups();
  }, [currentGroupQuery, groupsLoaded, season, viewMode]);
                  ...
                  <div style={{ display: "grid", gap: 8 }}>
                    ...
                    >
                      {Array.from({ length: 4 }).map((_, index) => (
                        ...
                        >
                          <UiSkeleton width={index % 2 === 0 ? "44%" : "58%"} height={16} />
                          <UiSkeleton width="32%" height={12} />
                        </div>
                      ))}
                    </div>
                  </div>
